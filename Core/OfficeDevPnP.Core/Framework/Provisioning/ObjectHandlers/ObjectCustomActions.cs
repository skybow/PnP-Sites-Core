            ...
            foreach (var customAction in customActions)
            {
                ...
                if (!caExists)
                {
                    var customActionEntity = new CustomActionEntity()
                    {
                        ...
                        ScriptSrc = parser.ParseString(customAction.ScriptSrc, TokenParser.SPSiteTokenKeys),
                ...
                else
                {
                    ...
                    if (existingCustomAction != null)
                    {
                        ...
                        if (existingCustomAction.ScriptSrc != parser.ParseString(customAction.ScriptSrc, TokenParser.SPSiteTokenKeys))
                        {
                            scope.LogPropertyUpdate("ScriptSrc");
                            existingCustomAction.ScriptSrc = parser.ParseString(customAction.ScriptSrc, TokenParser.SPSiteTokenKeys);
                            isDirty = true;
                        }
